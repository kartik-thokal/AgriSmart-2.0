            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                {fertilizerData.map((item) => {
                    ...
                    return (
                        ...
                        >
                            {/* Product Image */}
                            <Link to={`/fertilizer/${item.id}`} className="relative overflow-hidden bg-gray-50 flex items-center justify-center h-[220px]">
                                <img
                                    src={item.image}
                                    alt={item.name}
                            ...
                            <div className="p-5 flex flex-col flex-grow">
                                ...
                                <button
                                    onClick={() =>
                                        addToCart({
                                            id: `${item.id}-${item.sizes[0].label}`, // Defaults to first size (e.g., 1Kg)
                                            name: item.name,
                                            price: startingPrice,
                                            image: item.image,
